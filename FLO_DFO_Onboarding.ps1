...
Write-Host ""
Write-Host "`tTool    :: Script de configuration automatique de Defender For Office threat policies." -ForegroundColor Magenta
...
$ExcludedDomains = "", "" | Select-Object -Unique # QB Online has been getting popped for phishing lately. Highly reccomended to include Intuit.

$ExcludedSenders = "", "" | Select-Object -Unique
...
# Voir ligne 237 du code en cas d'activation
...
Write-Host -ForegroundColor $MessageColor "Setting up automatic forwarding from $CusAdminAddress > to > $MSPAlertAddress"
Write-Host
Write-Host
